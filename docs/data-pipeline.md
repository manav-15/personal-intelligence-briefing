...
Last updated: 2026-09-21. This is the pipeline implementation reference and
...
| Briefing publication and run status           | Manual and scheduled Workflow reserve/collect/compose/publish flow; normal same-origin run-status API with terminal failure reasons                       |
| Run-scoped collection and evidence            | SearXNG-only bounded collection, temporary evidence, exact URL dedupe, and contained provider failures                                                    |
| Google News RSS and GDELT discovery           | Retired 2026-09-21: adapters, decoder, verification scripts, and the Google redirect evidence branch were deleted; stored provenance names stay parseable |
...
| Ranking, grouping, briefing generation, chat  | Ranking/grouping, scheduled and manual generation, and durable source-scoped chat sessions implemented; evidence refresh remains planned                  |

The feasibility endpoint returns diagnostics, not briefings. It searches the
configured local SearXNG instance and reports the evidence outcome of each lead.
Preferences, topic proposals, and immutable published briefing payloads are
persisted. While a
...
| `publisher`    | SearXNG result source label, or the destination hostname             |
| `publishedAt`  | Parsed SearXNG date metadata; null when the engine returns none      |
| `sourceUrl`    | Direct publisher link returned by the search engine                  |
| `discoveryUrl` | Original discovery link when `sourceUrl` is a resolved publisher URL |
| `discovery`    | `searxng` for every new lead; retired names stay parseable           |
...
still parses `google-news` and `gdelt` because retained runs are re-read, but
nothing produces those values any more. `providerCalls` returns SearXNG alone,
and `briefing-workflow.ts` configures it from the local `SEARXNG_BASE_URL` or the
private Container, so a generate-briefing run has exactly one discovery channel.
Multiple configured engines supply the redundancy within it.
...
### Google News RSS — retired 2026-09-21

`discovery/google-news.ts`, `discovery/google-news-decoder.ts`, the Google
redirect branch of `evidence.ts`, and the `verify:google-links` script were
deleted. The measurements below are kept because they explain the decision: the
channel was the most expensive and least stable of the three, and SearXNG now
returns directly usable publisher links for the same topics.

Retired behaviour: `discovery/google-news.ts` validated query length (2–200), locale/country
...
### GDELT — retired 2026-09-21

`discovery/gdelt.ts` and the `verify:search-links` script were deleted. The
adapter was already unreachable from a normal run because a configured SearXNG
channel replaced it, and it never overcame the 429s recorded below.

Retired behaviour: `discovery/gdelt.ts` validated query length/result cap, appended an English
...
| DISC-05   | Medium               | Dropped         | Robust RSS/entity parsing                                       | Retired with the Google News RSS adapter on 2026-09-21; SearXNG returns JSON, so no Worker-side RSS parsing remains                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| DISC-06   | Deferred             | Partial         | Provider-side date filters                                      | A provider-side range is rejected by the evidence: SearXNG skips any engine without range support (`search/processors/abstract.py`, `get_params` returns `None`), and the one engine producing usable leads — `duckduckgo news` — does not support ranges, while the range-capable `reuters` produced 162 stale and 0 usable leads across five runs. Daily collection therefore keeps `timeRange: any` and the app-side two-day window stays authoritative. Revisit only if a range-capable engine starts producing fresh leads.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |     | DISC-08 | Conditional | Dropped   | Google News RSS publisher-link decoder | Deleted on 2026-09-21 with the Google News adapter. SearXNG returns direct publisher links, so no redirect decode step exists; the earlier decoder and its measurements are recorded in `architecture.md`                                                                                                                                                                                                                                                                                           |
| DISC-09   | High                 | Dropped         | Decode budget and freshness-first decoding                      | Retired with DISC-08 on 2026-09-21. `maxGoogleNewsDecodes` stays only in the persisted budget schema so stored run snapshots still parse; freshness gating now happens once, on search dates                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| DISC-10   | High                 | Partial         | SearXNG-only production discovery                               | SearXNG is the only provider with local Docker or private Container routing; the Google News/GDELT adapters, decoder, and verification scripts were removed on 2026-09-21. Tests cover partial engine errors, complete failure, budgets and provenance. Hosted startup is now verified: the deployed Container instance answered every Worker query from the Workflow. Remaining: hosted eligible-result yield per engine (DISC-12), which is what limits edition size.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| DISC-11   | Out of current scope | Deferred        | Feed and API discovery channel                                  | Not needed for the SearXNG-only direction in DISC-10.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| DISC-12   | High                 | Partial         | Hosted discovery yield                                          | Measured with `briefing.collected`, the retained diagnostics, and direct SearXNG probes across fifteen runs. Every usable lead comes from `duckduckgo news`; `reuters` (162 stale, 0 usable) and `brave.news` (64-67 undated per run, one recovered, dated volume crowding the shared window) are retired. Category and range arms over the same twelve queries: `news + any` returned 204 leads all dated; `news + day` returned none because the only working news engine has no range support and `bing news` is suspended; `general + any` returned 177 leads from `brave` (38% dated) after `duckduckgo` answered CAPTCHA on every query, and those leads were non-news pages (Wikipedia, Reddit, publisher help) that all answered 403 to evidence retrieval, so they could only be headline-only; `general + day` returned none because both general engines refused. Remaining: raise the evidence budget and fix `bing news` (suspended with connection and parsing errors, though it supplied 2 usable leads in one run). |     | EVID-04 | High        | Partial   | Description fallback                   | Strong same-story evidence wins; excluded/old/title-only stories rejected; labels/caps/grounded chat hold                                                                                                                                                                                                                                                                                                                                                                                           |
...
| BRIEF-04  | High                 | Completed       | Composition contradiction handling                              | An item-level contradiction no longer fails the edition: that selection is discarded, the remaining stories publish, and the edition carries one `composition-rejected` limitation naming the reason in plain language. Whole-run failure remains for unparseable model output, an empty candidate set, or no usable stories, and that message now names the discards. Tests cover every rejection category, the published subset, candidate reuse after a rejection, and dropping the lowest-relevance surplus above the story limit. Live: a run whose model mismatched 3 of 4 presentation topics published the valid story with that limitation instead of failing, and a run with no valid item failed with the discard reasons named. Remaining: repair call (BRIEF-06) and the mismatch root cause (BRIEF-07).                                                                                                                                                                                                               |
| BRIEF-05  | High                 | Dropped         | Invalid model grouping aborts the edition                       | Covered by BRIEF-04: an invalid grouping is discarded per item instead of aborting the run, and `story-budget` replaces the previous hard failure when the model returns more items than the story limit.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
| BRIEF-06  | High                 | Planned         | Bounded composition repair                                      | After items are discarded, at most one repair call re-asks the model with the stated reasons and only the unused candidates, within the run's existing evidence and token budgets. Accept when it returns at least one additional valid item; otherwise publish the subset unchanged.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| BRIEF-07  | High                 | Planned         | Deterministic presentation topic                                | Live runs keep producing items whose `presentationTopicId` is a valid topic that its own candidates do not belong to: 3 of 3 items on one run and 3 of 4 on another, with and without the added prompt rule. Derive the presentation topic in code when a group's candidates share one topic, and keep the model's choice only for genuinely multi-topic groups. Remove the class of failure rather than repairing it.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| STORE-01  | Persistence phase    | Partial         | Proposal/diagnostic retention and evidence TTL                  | Briefing deletion is decided and implemented: an owner may delete one edition or all editions; one SQLite transaction removes retained evidence, temporary candidates, briefing rows, and run rows, while retained chat sessions are detached and remain readable without source context. Proposal/diagnostic retention, Workflow retention, evidence TTL, and the 90-day deduplication-memory deletion remain to decide.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
...
| CHAT-01   | Chat phase           | Partial         | Persistent grounded follow-ups                                  | Agent-owned session/message rows, archive-aware session library, source-scoped saved-briefing answers that include the retained bounded extract, resilient WebSocket transport, and explicit deletion implemented; live evidence refresh and prior-coverage comparison remain. The turn now uses the retained extract together with the model's own general knowledge, which the answer must mark as Background rather than attribute to a source, and it states plainly that it has no internet access when asked to look something up. Attribution is instruction-driven: the code-owned append that used to add [S1] to any unlabelled answer is removed, because with knowledge allowed it fabricated provenance rather than repairing it, so the code-owned sources aside is now the only label surface and an unlabelled claim is untraceable                                                                                                                                                                                 |
| CHAT-02   | Medium               | Partial         | Recent chat history and UI reliability                          | Latest 12 model messages and latest 200 displayed messages now use chronological ordering after selecting the newest rows; regression tests cover both limits. UI serializes conversation actions, guards stale refreshes and displays errors. Remaining: exercise saved-conversation switching and transport failures in the browser; add pagination to expose older retained messages beyond the 200-message display window.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
| CHAT-03   | High                 | Partial         | Untrusted context boundary in chat prompts                      | One turn is sent as system policy, then the story as a single untrusted JSON data block, then the transcript ending at the current question. Article text, summary, change note and question stay out of the system message, and JSON escaping stops source text from forging a role or turn marker. Tests cover policy-only system content, turn ordering, an injection payload, and the call-site message array; a live browser turn still answered with [S1] attribution. Remaining: validate generated citation labels, rejecting a label the supplied source list does not contain.                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| TEST-01   | Before submission    | Partial         | Portable tests and final expanded-change validation             | Test alias resolves from import.meta.url; the complete `npm run check` gate passed on Node 24 with 209 tests, formatting, lint, strict type checks and a production build after the retired-provider removal, the chat prompt split and the composition rejection change. Remaining: verify the suite from a relocated checkout.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| DEPLOY-01 | Deployment phase     | Partial         | Private hosted SearXNG                                          | Deployed and exercised 2026-09-21: `SEARXNG_SECRET` set as a Worker secret, the pinned image pushed to the Cloudflare registry (`personal-intelligence-briefing-searxngcontainer`, `lite`, one instance), and the Worker published at the workers.dev hostname behind Access. The first hosted generations started Container instance `fa945c62` in `nrt10`, which answered every Worker query with `GET http://searxng.internal/search?... - Ok` before sleeping, so the private binding path works end to end. Remaining: hosted discovery yield (DISC-12) — the 12-query run produced one eligible candidate and seven collection failures, which published a one-item partial edition in one run and failed composition in another.                                                                                                                                                                                                                                                                                             |
| DEPLOY-02 | Deployment phase     | Partial         | Cloudflare Access protection                                    | JWT verification and production audience are configured. Owner data APIs and Agent routing enforce identity; static assets and health rely on hostname-level Access. Feasibility is now flag-gated and authenticated. Verify hostname policy and the changed routes after the separately reviewed deployment.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
...
| DEPLOY-04 | Later                | Partial         | Repository-driven infrastructure and deploys                    | The GitHub Actions gate exists and passes: `.github/workflows/ci.yml` runs `npm ci` and `npm run check` on Node 24 for every push. It had never passed before 2026-09-21 because `package-lock.json` was stale — `npm ci` reported 28 missing packages and eight nested `@babel` versions — so the lock was regenerated without changing a direct dependency. Remaining: run the deploy from CI, and manage the Access application and policy as code so the AUD tag is an output rather than a copy-paste; one-time account actions stay manual.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |     | SEC-01  | High        | Completed | Diagnostic and Agent route boundaries  | Feasibility requires the local inspection flag, identity, and same-origin policy; with the flag absent it answers 404 before any provider call. Agent paths enforce the personal-briefing binding and resolved owner on all subpaths and reject cross-origin HTTP and WebSocket requests. Live checks on the local Worker: owner handshake 101, foreign owner 404, foreign origin 403, Container binding 404, diagnostics-disabled 404 with owner APIs 401. Deployment remains separately reviewed. |
...
| DEV-02    | Later                | Planned         | Local `wrangler dev` logs a canceled request after a failed run | Under `wrangler dev`, a run that ends in composition failure is followed by `The Workers runtime canceled this request because it detected that your Worker's code had hung`. Observed 2026-09-21 on the scheduled tick and, identically, on the unchanged manual `POST /api/briefings/generate` path; a published run does not produce it. No hosted symptom has been seen, so this is scoped to the local dev harness: identify which request is canceled (the Workflow instance invocation is the likely candidate) before treating it as a product defect.                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
| SCHED-01  | Briefing phase       | Completed       | Daily briefing scheduling                                       | A `*/15 * * * *` Worker cron reserves one run per local calendar date once the saved `schedule.localTime` has arrived (due until local midnight), stamps `briefing_runs.trigger` (`manual` \| `scheduled`), and creates the Workflow without HTTP or Access; the owner id matches the HTTP path. Unconfigured, topic-less, early, already-published, and already-running ticks are silent no-ops; a failed run retries on a later tick, a published date never does, and manual generation still works and may publish a second edition the same day. Migration 10 adds the trigger column with `'manual'` for existing rows.                                                                                                                                                                                                                                                                                                                                                                                                       |
...
| UX-04     | Chat phase           | Completed       | Scrollable saved-chat library                                   | Superseded in increment 5.2 by `src/web/ChatDrawer.tsx`: saved conversations, New conversation, and Delete live in a modal drawer that is full width below 720px and right-anchored at 420px above it. It traps focus, closes on Escape and a backdrop tap, and the screen returns focus to its opener, so the panel can no longer obscure the transcript or its controls. The remaining responsive-shell work is UX-06                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
...
| UX-06     | Chat phase           | Planned         | Responsive application shell                                    | The shell header, six-link navigation, and footer render above and below every screen. Increment 5.2 left them unchanged and 5.3 hid only the footer on the chat screen, so the app header (43px) and the two-row navigation (105px at 375px wide) are now the binding constraint on the transcript: at 390x420 the transcript falls to its 4rem floor, the page scrolls 110px, and Send sits below the fold. A shell pass must keep Today, Archive, Topics, Memory & settings, and the Content lab usable at 320px and re-verify all six routes                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| UX-07     | Chat phase           | Planned         | Real-device verification of mobile chat                         | Increments 5.2 and 5.3 were verified only in emulated Chromium, and screenshot capture stopped responding in that environment, so the confirmation is geometric. Keyboard opening, rotation, scroll-into-view on focus, and drawer focus on iOS Safari and Android Chrome need a device or device cloud; the detached-conversation, no-edition, and load-failure screens also need a fixture, because the local Durable Object cannot produce them                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| UX-08     | Chat phase           | Completed       | Composer reachability and follow behaviour                      | Implemented in 5.3. The chat screen owns the viewport, the transcript is its only scroll region (`overscroll-behavior: contain`), and the composer is a fixed row below it, so the page does not scroll and nothing is sticky. The textarea starts at one line and grows to 8.5rem with internal scrolling. `chat-scroll.ts` follows only while the reader is within 120px of the end, `chat-viewport.ts` restarts following per conversation and re-aligns through a ResizeObserver as the composer grows, and Jump to latest appears in the composer while the reader is away. Measured transcript heights: 66/165/342/430px at 320x568/375x667/390x844/430x932                                                                                                                                                                                                                                                                                                                                                                   |
| UX-09     | Chat phase           | Completed       | Touch and reading polish for chat                               | Implemented in 5.3. Form controls render at 1rem so iOS Safari does not zoom on focus, buttons carry a 44px minimum, the composer label is screen-reader-only, and the shortcut hint follows the input device: Enter sends on a fine pointer while Return inserts a newline on a coarse one and Send submits. Verified with `(pointer: coarse)` emulated (Return produced two lines without sending) and on a fine pointer (Enter took the submit path)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
...
- DISC-02: partial; SearXNG snippets/provenance preserved. RSS descriptions no
  longer apply, because the RSS adapter was removed (DISC-05).
...
- DISC-07: completed for bounded manual generation. Discovery rotates through
  enabled topics, then queries; candidate caps and retrieval use a fair topic
  order. More enabled topics than available calls
  cannot all receive a pass; the shared cap holds and uncovered topics are
  disclosed rather than overspent.

- DISC-08 and DISC-09 (retired 2026-09-21): the decoder and its budget were
  deleted with the Google News adapter, so the paragraph below records a removed
  mechanism rather than current behaviour. The trigger was the 2026-09-19 evaluation, where SearXNG's
  `google news` engine was CAPTCHA-suspended and contributed no retained
  candidate. `discovery/google-news-decoder.ts` resolved a Google RSS item through
...
- SCHED-01: completed. `src/server/local-clock.ts` reads the calendar date and
  minutes-from-midnight in the saved timezone; `scheduled-briefing.ts` reserves
  and launches the run without HTTP; `reserveScheduledBriefingRun` performs the
  due-check, published-date check, and running-run coalescing inside one
  transaction. The Worker exports `{ fetch, scheduled }`, and `wrangler.jsonc`
  declares `*/15 * * * *`. Coverage: clock boundaries around 08:00 Asia/Kolkata
  and a changed timezone, plus orchestrator and reservation cases for
  not-configured, no-topics, not-due, already-published, already-running,
  workflow-start failure, and post-failure retry. Local validation used a
  deliberately triggered tick because `wrangler dev` does not fire cron on its
  own.
