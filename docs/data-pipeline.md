...
| DEPLOY-01 | Deployment phase     | Partial         | Private hosted SearXNG                                         | Private Container is configured without a public route; deployment validates image build, secret injection, engine coverage, and hosting-IP behavior, and must deploy with the repository config because the plugin redirect breaks the container image path                   |
| DEPLOY-02 | Deployment phase     | Planned         | Cloudflare Access protection                                   | Access application and single-identity policy exist before any hostname; the Worker verifies the Access JWT (RS256, JWKS, aud, iss, exp) and fails closed, maps service tokens to the same owner, and rejects unauthenticated API and Agent requests                           |
