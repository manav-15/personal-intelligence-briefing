...
| DISC-06   | Deferred             | Partial         | Provider-side date filters                                     | Inspection sends SearXNG `time_range`; daily collection deliberately does not until per-engine coverage is measured. Measured on 2026-09-21: with no range filter, 12 queries returned 95 leads and 89 of them were rejected as `stale`, so the missing filter is now the leading cause of thin editions. Next: send `time_range=day` for a daily run whose app-side window still allows the shared buffer day, and compare `outcomes` in the `briefing.collected` log before and after.                                                                                                                                                                                                                                                                                                                                                            |     | DISC-07 | High | Completed | Fair collection scheduler             | Round-robin topic/channel passes and evidence allocation stay within shared budgets; configured SearXNG runs first; exhausted capacity is disclosed |
| DISC-08   | Conditional          | Dropped         | Google News RSS publisher-link decoder                         | Deleted on 2026-09-21 with the Google News adapter. SearXNG returns direct publisher links, so no redirect decode step exists; the earlier decoder and its measurements are recorded in `architecture.md`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
...
| DISC-12   | High                 | Partial         | Hosted discovery yield                                         | Measured through the deployed Container and the local instance with the `briefing.collected` event: 12 queries answer normally and return 95 leads, `bing news` is the only failing engine (12-18 error entries per run), and the lead outcomes were `stale:89, description:4, article:1, headline-only:1`. Reachability is therefore not the limit; freshness is, and DISC-06 carries the fix. Remaining: re-measure `outcomes` after the range filter changes, then decide whether engine selection or query wording needs more work.                                                                                                                                                                                                                                                                                                             |     | EVID-01 | High | Partial   | Readable-body and challenge detection | Navigation/consent and clear index/timeline pages fail; checked publisher fixtures extract matching article body                                    |
