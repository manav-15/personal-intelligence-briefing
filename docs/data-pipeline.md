...
| DISC-12   | High                 | Partial         | Hosted discovery yield                                         | Measured with `briefing.collected` and the retained diagnostics across twelve runs. Every usable lead comes from `duckduckgo news`; `reuters` produced 162 stale and 0 usable leads (retired) and `brave.news` 64-67 undated leads per run of which the bounded date resolution recovered one (retired again after measurement). With both retired and `maxResultsPerQuery` at 12, a run returns 104 leads with 11 usable and drops 8 more at the evidence budget, which is now the binding limit. The cap, not the engine list, is what decides yield: an undated-heavy engine pushes the dated one out of the shared per-query window, and disabling Brave News restored usable leads from 5 to 11. Remaining: raise the evidence budget and fix `bing news` (suspended with connection and parsing errors, though it supplied 2 usable leads in one run) before judging query wording. |     | EVID-03 | High        | Partial | Reserved-address/redirect hardening    | IPv4/IPv6 and credentials tested; DNS/host policy documented for Worker runtime                                                                                                                           |
