...
| DEV-01    | High                 | Decision needed | Local development against an Access-protected deployment       | `npm run dev` no longer starts once the deployed Worker sits behind Access: the remote-binding session that `ai.remote` requires asks for `cloudflared` and exits. Choose between installing cloudflared, giving the dev session Access service-token credentials, or a dev-only configuration that skips the remote session, because local verification is otherwise blocked                                                      |
