...
| DISC-12   | High                 | Partial         | Hosted discovery yield                                         | Measured with `briefing.collected` and the retained diagnostics across seven runs. Every usable lead comes from `duckduckgo news` (1, 11, 4, 1, 3, 11, 11 per run); `reuters` produced 162 stale and 0 usable leads, so it is retired from `settings.yml`; `maxResultsPerQuery` is now 12 (raw pool 95 → 104 leads). With reuters off, the remaining stale noise is entirely `duckduckgo news` (~80% of its results are older than the window) and the run budget is the new binding limit: `evidence-budget: 8` leads were dropped after passing the date gate. Remaining: raise the evidence budget and fix `bing news` (suspended with connection and parsing errors, though it did supply 2 usable leads in one run) before judging query wording.                                                                                              |     | EVID-02 | High        | Completed | Stream Worker bounds and contain read failures | Oversized chunked responses stop early; broken streams return unavailable                                                                                                                                 |
