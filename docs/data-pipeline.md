...
| DISC-10   | High                 | Partial         | SearXNG-only production discovery                              | SearXNG is the only provider with local Docker or private Container routing; the Google News/GDELT adapters, decoder, and verification scripts were removed on 2026-09-21. Tests cover partial engine errors, complete failure, budgets and provenance. Live local probe returned five AI candidates while retaining a Bing engine error, and a production-style binding set returned 404 before any provider call. Remaining: verify hosted Container startup and generation across configured topics after deployment review.                                                                                                                                                                                                                                                                                                                     |
...
| DEPLOY-01 | Deployment phase     | Partial         | Private hosted SearXNG                                         | Deployed 2026-09-21: `SEARXNG_SECRET` set as a Worker secret, the pinned image pushed to the Cloudflare registry (`personal-intelligence-briefing-searxngcontainer`, `lite`, one instance), and the Worker version published at the workers.dev hostname behind Access. Remaining: run one hosted generation through the Container and record engine coverage, cold start and failure visibility (DISC-10). No public SearXNG route exists. |
