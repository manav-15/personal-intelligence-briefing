...
| DISC-07   | High                 | Completed       | Fair collection scheduler                                      | Round-robin topic/channel passes and evidence allocation stay within shared budgets; configured SearXNG runs first; exhausted capacity is disclosed                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
...
| DISC-12   | High                 | Planned         | Hosted discovery yield                                         | Measure per-engine eligible yield from the Container egress (dated, fresh, non-duplicate leads per query) rather than engine reachability alone, then tune `settings.yml` engine selection and topic query wording so a normal run can fill an edition. Evidence so far: 12 queries produced one eligible candidate and seven collection failures, publishing a one-item partial edition once and failing composition once, while every query returned HTTP 200 from SearXNG.                                                                                                                                                                                                                                                                                                                                                                       |     | DISC-11 | Out of current scope | Deferred | Feed and API discovery channel | Not needed for the SearXNG-only direction in DISC-10. |
